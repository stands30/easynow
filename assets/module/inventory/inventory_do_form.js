...
function getDOProducts()
{
  ...
  $.ajax(
        {
            type: "POST",
            url: baseUrl + "inventory/getDOProducts/"+piv_type_id,
            dataType: "json",
            success: function(response)
            {
                 ...
                 for (i = 0; i < response.length; i++) 
                 {
                   ...
                   var prodSizeSelect2Val =
                     ...
                     if(response[i]["unblocked_stock"] == '')
                        ...
                        {
                          $('input[name="product_repeater_item['+i+'][piv_qty]"]').val(response[i]["unblocked_stock"]).addClass('edited');
                          $($('.piv_pending_span')[i]).html(response[i]["unblocked_stock"]+' / '+response[i]["piv_pending"]);
                          $($('.piv_pending_span')[i]).attr('data-piv_total',response[i]["unblocked_stock"]);
                          $($('.piv_total_span')[i]).html(response[i]["total_stock"]);
                        }
